...
SetTVAudio() {
    SetDefaultEndpoint(GetDeviceID(List, "D43-C1 (AMD High Definition Audio Device)"))
    TrayTip "", "Set audio device to TV"
}

SetSpeakersAudio() {
    SetDefaultEndpoint(GetDeviceID(List, "Speakers (High Definition Audio Device)"))
    TrayTip "", "Set audio device to Speakers"
}
...
If (WinWaitActive("ahk_exe PlexMediaPlayer.exe")) {
    SetTVAudio()
}
